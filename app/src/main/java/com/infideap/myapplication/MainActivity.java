package com.infideap.myapplication;
...
import com.infideap.myapplication.dummy.DummyContent;
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        if (id == R.id.nav_camera) {
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.content_main1, BlankFragment.newInstance(null, null))
                    .commitAllowingStateLoss();
            // Handle the camera action
        } else if (id == R.id.nav_gallery) {
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.content_main2, ItemFragment.newInstance(0))
                    .commitAllowingStateLoss();

        } else if (id == R.id.nav_slideshow) {

            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.content_main1, ItemFragment.newInstance(0))
                    .commitAllowingStateLoss();

        } else if (id == R.id.nav_manage) {
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.content_main2, BlankFragment.newInstance(null, null))
                    .commitAllowingStateLoss();
